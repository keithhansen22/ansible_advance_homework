...
* Ran into an issue with Openstack; network, router, port resource provisioning - something got corrupt and about lost environment so had to rebuild. Ended up creating new osp-setup directory to run clean config - as something in homework config kept creating a problem had to run a bunch of openstack cmds to resolve and then use these new osp-setup files to rebuild infra/instances. This was communicated with instructor jkupfere. 
